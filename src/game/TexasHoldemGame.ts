...
export class TexasHoldemGame {
	...
	addPlayer(
		...
		logger.info(
			`Player '${name}' (${newPlayerId}) joined as ${isSpectator ? "spectator" : "player"}. Total players: ${this.players.length}`,
		);
		if (newPlayer.status === "sitting-out") {
			...
			this.sendToPlayer(
				newPlayer,
				"info",
				`Your ID: ${newPlayerId}. You are a ${isSpectator ? "spectator" : "player"}.`,
			);
		}
	...
	private startNewHand(): void {
		...
		this.serverMessage = `New hand started. Dealer: ${this.players[this.dealerIndex].name}, SB: ${this.players[sbIndex].name}, BB: ${this.players[bbIndex].name}.`;
		this.broadcastState();
		logger.info(
			`New hand started. Dealer: ${this.players[this.dealerIndex].name}, SB: ${this.players[sbIndex].name}, BB: ${this.players[bbIndex].name}.`,
		);
	}
	...
	private dealHoleCards(): void {
		this.players
			...
			.forEach((player) => {
				...
				logger.info(
					`Dealt hole cards to ${player.name}: [${player.hand.map((c) => c.rank + c.suit).join(", ")}]`,
				);
			});
	}
	...
	private promptCurrentPlayerAction(): void {
		...
		this.serverMessage = `It's ${currentPlayer.name}'s turn. Bet to call: ${chipsToCall}. You can check if ${chipsToCall === 0}. Minimum raise is ${this.minimumBetForCall + this.lastRaiseAmount}.`;
		this.broadcastState();
		logger.info(
	...
	private moveToNextPlayer(): void {
		...
		while (!nextPlayerFound && checkedCount < this.players.length) {
			...
			if (nextPlayer.status === "active" && nextPlayer.hand.length > 0) {
				if (
	...
	handlePlayerAction(
		...
		switch (actionType) {
			...
			case "raise": {
				if (amount === undefined || amount <= this.minimumBetForCall) {
					this.sendToPlayer(
						player,
						"error",
						`Raise amount must be greater than current bet (${this.minimumBetForCall}).`,
					);
					logger.warn(
				...
				if (
					totalBetAfterRaise < minimumRaiseRequired &&
					player.chips > totalBetAfterRaise - player.currentBet
				) {
					this.sendToPlayer(
						player,
						"error",
	...
	private dealCommunityCards(count: number): void {
		...
		logger.info(
			`Dealt ${count} community cards: [${newCards.map((c) => c.rank + c.suit).join(", ")}]`,
		);
		...
		playersInShowdown.forEach((player) => {
			...
			this.players.forEach((p) => {
				this.sendToPlayer(
					p,
					"info",
					`${player.name}'s hand: [${player.hand.map((c) => c.rank + c.suit).join(", ")}] Best 5-card: ${handRank.type} with [${handRank.cards.map((c) => c.rank + c.suit).join(", ")}]`,
				);
			});
			logger.info(
